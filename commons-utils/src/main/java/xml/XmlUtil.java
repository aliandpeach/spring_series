...
/**
 * @ClassName: ConfigPropertiesSrv
 * @Description: DOM工具类，处理XML
 * @author: tanyp
...
public class XmlUtil {
    ...
    public static void save(Document doc, String xmlPath, String encoding) throws Exception {
        ...
        XMLWriter writer = new XMLWriter(new OutputStreamWriter(Files.newOutputStream(Paths.get(xmlPath)), encoding), format);
        writer.write(doc);
        writer.flush();
        writer.close();
    }
    ...
    public static void modifyDocument(File inputXml, String nodes, String attributename, String value, String outXml) {
        try {
            ...
            logger.error("modifyDocument error", e);
        } catch (IOException e) {
            logger.error("modifyDocument error", e);
        }
    }
