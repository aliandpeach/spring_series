...
public class AnnotationWebsiteControllerExceptionHandler
{
    private static final Logger logger = LoggerFactory.getLogger(AnnotationWebsiteControllerExceptionHandler.class);
    ...
    public BaseResponse<?> handleDataIntegrityViolationException(DataIntegrityViolationException e)
    {
        ...
        baseResponse.setMessage("字段验证错误");
        return baseResponse;
    }
    ...
    public BaseResponse<?> handleBindException(BindException e)
    {
        ...
        if (e.hasErrors() && e.getFieldErrors().size() > 0)
        {
...
//        baseResponse.setMessage("缺失请求主体");
    ...
    @ExceptionHandler(AnnotationWebsiteException.class)
    // @ResponseStatus(HttpStatus.BAD_REQUEST) // http协议返回的status code 就是 @ResponseStatus指定的值, 不指定一律返回 200
    public BaseResponse<?> annotationWebsiteException(AnnotationWebsiteException e)
    {
